...
export function KeepsakeCabinet({ onClick }: { onClick?: () => void }) {
  return <group position={[5.65,0,-6.18]} onClick={clickThrough(onClick)}><mesh position={[0,1.42,0]} castShadow receiveShadow><boxGeometry args={[3.15,2.84,0.65]} /><meshStandardMaterial color="#3c291f" roughness={0.48} /></mesh><mesh position={[0,1.42,0.34]} raycast={() => null}><boxGeometry args={[2.82,2.54,0.025]} /><meshPhysicalMaterial color="#eff7f3" transmission={0.94} transparent opacity={0.08} roughness={0.04} metalness={0.02} thickness={0.04} /></mesh>{[-1.42,0,1.42].map((x)=><mesh key={x} position={[x,1.42,0.37]}><boxGeometry args={[0.045,2.6,0.055]} /><meshStandardMaterial color="#c2a36e" metalness={0.72} roughness={0.2} /></mesh>)}{[0.64,1.38,2.14].map((y)=><mesh key={y} position={[0,y,0.2]}><boxGeometry args={[2.74,0.045,0.46]} /><meshStandardMaterial color="#7e6246" roughness={0.5} /></mesh>)}{[0.95,1.7,2.43].map((y)=><mesh key={`light-${y}`} position={[0,y,0.31]}><boxGeometry args={[2.45,0.018,0.025]} /><meshBasicMaterial color="#ffd89b" toneMapped={false} /></mesh>)}<pointLight position={[0,2.42,0.42]} intensity={0.72} distance={3.2} color="#ffd9a1" /><pointLight position={[0,1.2,0.42]} intensity={0.5} distance={2.7} color="#ffd9a1" /></group>;
}

export function PajamaWardrobe({ onClick }: { onClick?: () => void }) {
  return <group position={[-5.25,0,6.18]} rotation={[0,Math.PI,0]} onClick={clickThrough(onClick)}><mesh position={[0,1.55,0]} castShadow receiveShadow><boxGeometry args={[2.8,3.1,0.72]} /><meshStandardMaterial color="#4a3429" roughness={0.5} /></mesh><mesh position={[0,1.58,0.39]}><boxGeometry args={[2.45,2.75,0.04]} /><meshStandardMaterial color="#b99c76" roughness={0.78} /></mesh><mesh position={[0,2.66,0.46]}><cylinderGeometry args={[0.025,0.025,1.9,14]} /><meshStandardMaterial color="#c7aa76" metalness={0.72} roughness={0.2} /></mesh><pointLight position={[0,2.62,0.5]} intensity={0.35} distance={2.6} color="#ffdbac" /></group>;
}

export function DeskKeepsakes() {
  return <group position={[5.98,0.78,4.18]} rotation={[0,-Math.PI/2,0]}><mesh position={[0,0.025,0]} rotation={[0.03,0.1,-0.04]} castShadow><boxGeometry args={[0.58,0.055,0.42]} /><meshStandardMaterial color="#745f4b" roughness={0.78} /></mesh><mesh position={[0.02,0.065,-0.02]} rotation={[0.03,0.1,-0.04]}><boxGeometry args={[0.52,0.018,0.36]} /><meshStandardMaterial color="#e8dcc7" roughness={0.92} /></mesh><mesh position={[0.28,0.08,0.04]} rotation={[0,0,-0.32]}><cylinderGeometry args={[0.012,0.012,0.48,10]} /><meshStandardMaterial color="#b79868" metalness={0.45} roughness={0.3} /></mesh></group>;
}
